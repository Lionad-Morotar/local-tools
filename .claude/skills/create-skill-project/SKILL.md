...
description: 创建新的 Claude Skill 项目。
...
**注意**，技能名称不应当有后缀“-skills”，项目名称才允许有（注意去重，避免出现 xxx-skills?-skills 这种情况）

## 工作流程

1. 初始化仓库 `gh repo create <project-name> --public --description "<description>"`
2. 添加子包 `git submodule add https://github.com/Lionad-Morotar/<project-name>.git <project-name>`
3. 如果 `~/.claude/skills/<skill-name>` 已存在：
  3.1 复制其内容到 submodule
  3.2 删除目录，重建 `~/.claude/skills/<skill-name>` 软链指向真实技能（包含 SKILL.md 的父文件夹）
4. 子包和当前仓库分别提交并推送
