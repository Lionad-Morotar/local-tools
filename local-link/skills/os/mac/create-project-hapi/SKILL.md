...
> 注意：这里使用 `command grep` 绕过可能存在的 `grep` → `rg` shadow，避免正则解析错误。> 注意：`hapi` CLI 要求 Claude 参数出现在子命令之后，wrapper 会自动重排，因此 `hapi ckh --continue hub --relay` 等价于 `hapi ckh hub --relay --continue`。
